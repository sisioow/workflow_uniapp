...
- 代码生成工具在第 **6** 步选择：Claude Code（默认）/ Codex / Cursor Agent / Antigravity CLI
...
flowchart LR
  ...
  Orch --> CLI[Claude / Codex / Cursor / Antigravity CLI]
...
**需求分析**  
- 由 OpenAI 兼容接口根据应用名产出三套 `RequirementPlan`（方向 + 摘要 + page1～page4）。三套方向必须实质不同；**每一套**须是可独立使用的完整工具（每页 5～8 条完整功能点，并写清必要二级页），禁止只有一两条简单小功能。  
...
| **Antigravity CLI** | `ANTIGRAVITY_BIN` / `ANTIGRAVITY_MODEL` | Antigravity `agy` 非交互 print 生成 |
...
| Node / 各 CLI | 按所选代码生成工具安装 `claude` / `codex` / `agent` / `agy` |
